...
class _CreateMatchScreenState extends ConsumerState<CreateMatchScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: user == null
          ...
          : StreamBuilder(
              stream:
                  ref.watch(teamRepositoryProvider).watchUserTeams(user.uid),
              builder: (context, snapshot) {
                ...
                if (teams.length < 2) {
                  return EmptyStateView(
                    ...
                    message:
                        'Create two teams first, then you can schedule a match between them.',
                ...
                return ListView(
                  ...
                  children: [
                    ...
                    Text(
                      'Select teams and match format. You will select the starting XI on the next screen.',
                      style: Theme.of(context)
                          .textTheme
                          .bodyMedium
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      onChanged: (v) {
                        if (v == null) return;
                        final team = teams.firstWhere((t) => t.id == v);
                        setState(() {
                          _teamAId = v;
                    ...
                    ElevatedButton.icon(
                      ...
                      label: Text(
                        _loading ? 'Creating...' : 'Continue to Players',
                      ),
                    ),
                  ],
                );
  ...
  Future<void> _create(String userId) async {
    ...
    try {
      ...
      if (mounted) {
        context.go('/matches/${match.id}/squad');
      }
    } catch (e) {
      if (mounted) {
        setState(() => _error = 'Failed to create match. Please try again.');
      }
    } finally {
